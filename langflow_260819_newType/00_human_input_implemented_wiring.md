...
20 Human Input
  Approve  -> 18A / execution start
  Fallback -> 18A / execution start
...
- Keeps the official static `prompt` field and also supports `prompt_message` for dynamic `Message` wiring from 08H.
- Pauses the graph by calling `graph.request_pause(...)`.
- Stops every non-selected output branch after resume.
- Carries `payload_json` as a hidden `Data` input and emits it only through the selected branch.
...
```text
08 payload -> 20 Human Input.payload_json
