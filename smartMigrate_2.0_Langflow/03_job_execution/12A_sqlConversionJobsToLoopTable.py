...
class NewType12ASqlConversionJobsToLoopTable(Component):
    ...
    # payload 목록에 같은 SQL job이 여러 번 들어와도 Conversion Loop에는 한 번만 넣는다.
        ...
        for job in jobs:
            ...
            if key in seen:
                logging.getLogger("smartmigrate.workflow").warning(
                    "Duplicate SQL Conversion job removed before loop execution: key=%s", key
                )
                continue
            seen.add(key)
            unique.append(job)
    ...
    def _validate_sql_key(self, job: dict[str, Any], index: int) -> None:
        if str(job.get("sql_seq") or "").strip() or (str(job.get("space_nm") or "").strip() and str(job.get("sql_id") or "").strip()):
            return
        raise ValueError(f"12A SQL Conversion job row {index} requires sql_seq or space_nm+sql_id")
