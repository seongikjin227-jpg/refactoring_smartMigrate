...
class NewType12DSqlConversionIterationDashboard(Component):
    ...
    def build_message(self) -> Message:
        logging.getLogger("smartmigrate.workflow").info("before build_message", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "INFO", "BUILD_MESSAGE", "START", 0]})
        try:
            payload = self._build()
            self.status = payload
            __log_result = Message(text=str(payload.get("answer_text") or ""))
            logging.getLogger("smartmigrate.workflow").info("after build_message", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "INFO", "BUILD_MESSAGE", "END", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error build_message: {exc}", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "ERROR", "BUILD_MESSAGE", "ERROR", 0]})
            raise

    # iteration dashboard가 사용할 loop 결과 payload를 Data로 반환한다.
    def build_loop_result(self) -> Data:
        logging.getLogger("smartmigrate.workflow").info("before build_loop_result", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "INFO", "BUILD_LOOP_RESULT", "START", 0]})
        try:
            payload = self._build()
            self.status = payload
            __log_result = Data(data=payload.get("loop_result") or payload)
            logging.getLogger("smartmigrate.workflow").info("after build_loop_result", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "INFO", "BUILD_LOOP_RESULT", "END", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error build_loop_result: {exc}", extra={"workflow_log": [0, "WORKFLOW", "12D_SQL_DASH", "ERROR", "BUILD_LOOP_RESULT", "ERROR", 0]})
            raise
    ...
    # attempt history를 dashboard에 표시할 단계별 Markdown 줄로 변환한다.
    ...
    # attempt 한 건의 상태, retry, 오류 메시지를 짧은 표시 문자열로 만든다.
    ...
    # iteration dashboard에 표시할 현재 작업 status를 payload에서 결정한다.
