...
class NewType12CSqlConversionOneJobPocExecutor(Component):
    ...
    def _run_conversion_graph(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        def generate_tobe_node(state: dict[str, Any]) -> dict[str, Any]:
            if state.get("resume_stage") != "GENERATE_TOBE_SQL" and state.get("to_sql"):
                ...
                return state
            try:
                tobe_reuse_stage = "GENERATE_TOBE_SQL"
        ...
        def generate_bind_node(state: dict[str, Any]) -> dict[str, Any]:
            ...
            if state.get("resume_stage") == "GENERATE_TEST_SQL":
                ...
                return state
            try:
                bind_reuse_stage = "GENERATE_BIND_SQL"
        ...
        def generate_test_node(state: dict[str, Any]) -> dict[str, Any]:
            if state["tag_kind"] != "SELECT":
                ...
                return state
            try:
                test_reuse_stage = "GENERATE_TEST_SQL"
    ...
    #
    # SM_RAG_RULES.dense_vector는 04_saveVectorDB가 NEXT_MIG_RAG_INFO.SOURCE_SQL에서 생성한다.
    ...
    # -------------------------------------------------------------------------
    # 04_saveVectorDB가 NEXT_SQL_INFO에서 만든 SM_CORRECT_SQL_CONVERSION을 검색한다.
    ...
    def _correct_sql_hints_text(self, db_config: dict[str, Any], source_sql: str, current_sql_id: str | None, current_space_nm: str | None, map_id: str, retry_count: int, tag_kind: Any = "", ref_seq: Any = None) -> dict[str, str]:
        ...
        try:
            ...
            rows = self._milvus_client().search(
                ...
                output_fields=["sql_seq", "space_nm", "sql_id", "source_sql", "to_sql", "bind_sql", "test_sql", "correct_sql_kind", "status_conversion", "user_edited", "tag_kind"],
                search_params={"metric_type": "COSINE"},
            )
    ...
    def _referenced_correct_sql_hints(self, db_config: dict[str, Any], ref_seq: Any) -> dict[str, str]:
        ...
        rows = self._milvus_client().query(
            ...
            output_fields=["sql_seq", "space_nm", "sql_id", "source_sql", "to_sql", "bind_sql", "test_sql", "correct_sql_kind"],
            limit=1,
        )
