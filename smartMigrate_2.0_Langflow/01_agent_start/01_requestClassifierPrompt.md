...
- 예: 직전 대화가 "BIND Correct SQL SQL_SEQ=42를 FAIL-BIND 후보에 적용할까요?"이고 현재 입력이 "네"이면 `resolved_user_request`는 "SQL_SEQ=42 BIND Correct SQL을 FAIL-BIND 후보에 REF_SEQ로 적용하고 RETRY_COUNT를 0으로 변경해줘"입니다. `route=MANAGEMENT`, `confirmation=CONFIRMED`입니다.
...
- 유사 SQL 검색 결과의 `FAIL-*` 행을 "재시도 상태로 바꿔줘"라는 요청도 MANAGEMENT입니다. 이 요청은 FAIL 상태를 보존하고 `RETRY_COUNT`만 0으로 변경하며 실제 SQL 실행은 포함하지 않습니다.
