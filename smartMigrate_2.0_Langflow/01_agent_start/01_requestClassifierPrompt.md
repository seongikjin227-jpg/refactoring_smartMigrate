...
- VectorDB, Milvus, 벡터DB, 벡터 디비, vector upload, vector sync, RAG 벡터 동기화, 04_saveVectorDB 실행, 04_saveVectorDB 업로드, 04_saveVectorDB 동기화 요청은 MANAGEMENT입니다.
- "벡터DB 동기화해줘", "VectorDB 업로드해줘", "Milvus에 반영해줘", "방금 추가한 가이드를 벡터DB에 올려줘", "04 VectorDB 동기화 실행해줘"는 작업 실행(JOB_EXECUTION)이 아니라 MANAGEMENT입니다.
