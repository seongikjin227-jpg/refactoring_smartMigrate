...
- After explicit confirmation, build `retry_failed_sql_conversion` actions using only the confirmed `SQL_ID` + `SPACE_NM` pairs from the search result. Do not expect or request a separate `retry_actions` field. Never use `reset_sql_conversion_status`, `reset_sql_tuning_status`, or `retry_failed_sql_tuning` for this flow.
