...
VECTOR_DB_SYNC 규칙:
- 사용자가 "VectorDB 업로드", "Milvus 업로드", "RAG 벡터 동기화", "04 VectorDB 동기화 실행", "가이드 추가 후 벡터DB 반영"처럼 요청하면 이 route를 선택하세요.
- 이 route는 NEXT_MIG_RAG_INFO, NEXT_SQL_INFO, NEXT_MIG_INFO 원천 데이터를 04_saveVectorDB.py에서 Milvus collection으로 동기화하는 작업입니다.
- 사용자가 특정 RAG_ID만 말해도 현재 04_saveVectorDB는 전체 snapshot 동기화 방식이므로 부분 업로드 조건을 만들지 마세요.
...
class NewType04ManagementRouter(Component):
    ...
    def _next_node(self, route: str) -> str:
        return {"DASHBOARD": "04_dashboard", "CURRENT_PROGRESS": "04_currentProgress", "JOB_QA": "04_jobQaAgent", "STATUS_CHANGE": "04_statusChange", "CORRECT_SQL_INPUT": "04_correctSqlInput", "RAG_GUIDE_MANAGEMENT": "04_ragGuideManager", "VECTOR_DB_SYNC": "04_saveVectorDB", "EXCEPTION": "04_managementRouter"}.get(route, "04_dashboard")
