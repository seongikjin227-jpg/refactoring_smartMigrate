...
## SQL Formatting Override

- SQL Formatting 단독 실행 요청은 DB Migration, SQL Conversion, SQL Tuning 잔여 작업이 남아 있어도 `PREREQUISITE_REQUIRED`로 차단하지 않는다.
- `sql_formatting_total > 0`이면 현재 SQL Formatting 잔여 작업을 그대로 실행한다.
- 이 규칙은 전체 workflow 순차 실행이 아니라 사용자가 명시적으로 SQL Formatting 전체/잔여 실행을 요청한 경우에만 적용한다.
