...
def _workflow_log(step_name: str, status: str, message: str, log_level: str = "INFO") -> None:
    logging.getLogger("smartmigrate.workflow").log(logging.ERROR if str(log_level).upper() == "ERROR" else logging.INFO, str(message or ""), extra={"workflow_log": [0, "WORKFLOW", "MULTI_TOOL_06_SQL_TUNING_COMMAND_TOOL", str(log_level or "INFO").upper(), str(step_name or "")[:50], str(status or "")[:20], str(message or "")[:4000], 0]})

class PocSqlTuningCommandTool(Component):
    display_name = "POC SQL Tuning Command Tool"
    description = "POC tool for SQL Tuning. It returns PASS when the expected command is called."
    name = "poc_sql_tuning_command_tool"
    ...
    inputs = [
        MessageTextInput(
            ...
            required=True,
            tool_mode=True,
            info='Expected JSON: {"action":"run_sql_tuning_job","space_nm":"SFA","sql_id":"selectUser"}',
        ),
    ]
        ...
        try:
            ...
            if action != "run_sql_tuning_job":
                ...
                result = {
                    ...
                    "poc": True,
                    "message": "PASS: SQL Tuning POC tool was called. No real SQL tuning was executed.",
