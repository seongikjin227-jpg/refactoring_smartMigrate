...
class NewType15DSqlTuningIterationDashboard(Component):
    ...
    def build_message(self) -> Message:
        self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "INFO", "BUILD_MESSAGE", "START", "before build_message", 0, "")
        try:
            """Return a chat-friendly iteration message."""
            payload = self._build()
            self.status = payload
            __log_result = Message(text=str(payload.get("answer_text") or ""))
            self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "INFO", "BUILD_MESSAGE", "END", "after build_message", 0, "")
            return __log_result
        except Exception as exc:
            self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "ERROR", "BUILD_MESSAGE", "ERROR", f"error build_message: {exc}", 0, "")
            raise

    def build_loop_result(self) -> Data:
        self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "INFO", "BUILD_LOOP_RESULT", "START", "before build_loop_result", 0, "")
        try:
            """Return the loop aggregation payload."""
            payload = self._build()
            self.status = payload
            __log_result = Data(data=payload.get("loop_result") or payload)
            self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "INFO", "BUILD_LOOP_RESULT", "END", "after build_loop_result", 0, "")
            return __log_result
        except Exception as exc:
            self._insert_log(0, "WORKFLOW", "15D_SQL_DASH", "ERROR", "BUILD_LOOP_RESULT", "ERROR", f"error build_loop_result: {exc}", 0, "")
            raise
