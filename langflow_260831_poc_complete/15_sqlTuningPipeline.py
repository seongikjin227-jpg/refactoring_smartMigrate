...
class NewType15SqlTuningPipeline(Component):
    ...
    def run_pipeline(self) -> Data:
        ...
        logging.getLogger("smartmigrate.workflow").info("before run_pipeline", extra={"workflow_log": [0, "WORKFLOW", "15_PIPELINE", "INFO", "RUN_PIPELINE", "START", "before run_pipeline", 0]})
        try:
            ...
            logging.getLogger("smartmigrate.workflow").info("after run_pipeline", extra={"workflow_log": [0, "WORKFLOW", "15_PIPELINE", "INFO", "RUN_PIPELINE", "END", "after run_pipeline", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error run_pipeline: {exc}", extra={"workflow_log": [0, "WORKFLOW", "15_PIPELINE", "ERROR", "RUN_PIPELINE", "ERROR", f"error run_pipeline: {exc}", 0]})
            raise
