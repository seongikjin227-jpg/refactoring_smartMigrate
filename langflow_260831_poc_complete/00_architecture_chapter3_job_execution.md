...
flowchart TD
    ...
    R08 -->|FULL_WORKFLOW| W18[18A -> 18S -> 18B -> 10C/12C/15C/17C -> 18D]
...
sequenceDiagram
    ...
    participant S18 as 18S Start Logger
    ...
    A18-->>S18: Full Workflow DataFrame
    S18->>DB: workflow start log
    S18-->>B18: DataFrame with workflow metadata
...
| 7 | `18S_workflowStartLogger.py` | DataFrame | `MIG -> SQL_CONVERSION -> SQL_TUNING -> SQL_FORMATTING` 계획 count와 start event 로그 | workflow metadata가 포함된 DataFrame |
| 8 | `18B_fullWorkflowLoop.py` | DataFrame | 한 row씩 item output. route 순서 보존 | `job_item` |
| 9 | `10C/12C/15C/17C` | `job_item` | 각 도메인 단일 작업 실행, DB update, log insert | `job_result` |
| 10 | `18D_fullWorkflowDashboard.py` | `job_result` | iteration progress 메시지와 loop feedback 생성 | `loop_result` |
| 11 | `18B_fullWorkflowLoop.py` | loop feedback | 다음 row 진행, 끝나면 `loop_done=True` | done payload |
| 12 | `18D_fullWorkflowDashboard.py` | done payload | route별 planned/completed/pass/fail/skipped summary | final chat output |
...
`18A`, `18B`, `18S`, `18D`의 공통 route order는 다음이다.
