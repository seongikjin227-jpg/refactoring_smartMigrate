...
class NewType17BSqlFormattingLoop(Component):
    ...
    async def _iterate(self) -> list[Data]:
        ...
        try:
            ...
            self.log(f"SQL Formatting loop failed after {elapsed:.3f}s: {exc}", name="Error")
    ...
    async def item_output(self) -> Data:
        self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "INFO", "ITEM_OUTPUT", "START", "before item_output", 0, "")
        try:
            ...
            self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "INFO", "ITEM_OUTPUT", "END", "after item_output", 0, "")
            return __log_result
        except Exception as exc:
            self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "ERROR", "ITEM_OUTPUT", "ERROR", f"error item_output: {exc}", 0, "")
            raise

    async def done_output(self) -> Data:
        self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "INFO", "DONE_OUTPUT", "START", "before done_output", 0, "")
        try:
            ...
            self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "INFO", "DONE_OUTPUT", "END", "after done_output", 0, "")
            return __log_result
        except Exception as exc:
            self._insert_log(0, "WORKFLOW", "17B_SQL_LOOP", "ERROR", "DONE_OUTPUT", "ERROR", f"error done_output: {exc}", 0, "")
            raise
