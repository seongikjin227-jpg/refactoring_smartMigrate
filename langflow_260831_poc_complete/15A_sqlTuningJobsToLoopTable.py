...
class NewType15ASqlTuningJobsToLoopTable(Component):
    ...
    def build_jobs_table(self) -> DataFrame:
        logging.getLogger("smartmigrate.workflow").info("before build_jobs_table", extra={"workflow_log": [0, "WORKFLOW", "15A_SQL_JOBS", "INFO", "BUILD_JOBS_TABLE", "START", "before build_jobs_table", 0]})
        try:
            ...
            logging.getLogger("smartmigrate.workflow").info("after build_jobs_table", extra={"workflow_log": [0, "WORKFLOW", "15A_SQL_JOBS", "INFO", "BUILD_JOBS_TABLE", "END", "after build_jobs_table", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error build_jobs_table: {exc}", extra={"workflow_log": [0, "WORKFLOW", "15A_SQL_JOBS", "ERROR", "BUILD_JOBS_TABLE", "ERROR", f"error build_jobs_table: {exc}", 0]})
            raise
    ...
    def _sql_jobs(self, payload: dict[str, Any], db_config: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        jobs = payload.get("selected_jobs") or requested.get("sql_tuning_jobs") or payload.get("planned_jobs") or []
        ...
        for job in jobs:
            if not isinstance(job, dict):
                continue
            route = str(job.get("job_route") or "SQL_TUNING").upper()
            if route == "SQL_TUNING":
                out.append(dict(job))
        return out
    ...
    def _validate_sql_key(self, job: dict[str, Any], index: int) -> None:
        ...
        if str(job.get("space_nm") or "").strip() and str(job.get("sql_id") or "").strip():
            return
        raise ValueError(f"15A SQL Tuning job row {index} requires row_id or space_nm+sql_id")
    ...
    def _require_db_config(self, db_config: dict[str, Any]) -> None:
        """Fail early when SQL Tuning is not wired to database settings."""
        missing = [key for key in ("db_host", "db_service_name", "db_username") if not str(db_config.get(key) or "").strip()]
        if missing:
            raise ValueError(f"15A SQL Tuning is not connected to database settings: missing {', '.join(missing)}")
