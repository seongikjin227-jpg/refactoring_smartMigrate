...
class NewType15BSqlTuningLoop(Component):
    ...
    name = "NewType15BSqlTuningLoop"
    icon = "Infinity"

    inputs = [
        HandleInput(
            name="data",
            display_name="SQL Tuning Jobs",
            info="SQL tuning job rows to iterate.",
    ...
    async def _iterate(self) -> list[Data]:
        ...
        try:
            ...
            await logger.aexception(f"SQL Tuning loop {self._id} failed while executing loop body")
    ...
    async def item_output(self) -> Data:
        logging.getLogger("smartmigrate.workflow").info("before item_output", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "INFO", "ITEM_OUTPUT", "START", "before item_output", 0]})
        try:
            ...
            logging.getLogger("smartmigrate.workflow").info("after item_output", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "INFO", "ITEM_OUTPUT", "END", "after item_output", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error item_output: {exc}", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "ERROR", "ITEM_OUTPUT", "ERROR", f"error item_output: {exc}", 0]})
            raise

    async def done_output(self) -> Data:
        logging.getLogger("smartmigrate.workflow").info("before done_output", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "INFO", "DONE_OUTPUT", "START", "before done_output", 0]})
        try:
            ...
            logging.getLogger("smartmigrate.workflow").info("after done_output", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "INFO", "DONE_OUTPUT", "END", "after done_output", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error done_output: {exc}", extra={"workflow_log": [0, "WORKFLOW", "15B_SQL_LOOP", "ERROR", "DONE_OUTPUT", "ERROR", f"error done_output: {exc}", 0]})
            raise
    ...
    def _validate_sql_key(self, payload: dict[str, Any], index: int) -> None:
        ...
        if str(payload.get("space_nm") or "").strip() and str(payload.get("sql_id") or "").strip():
            return
        raise ValueError(f"15B SQL Tuning item {index} requires row_id or space_nm+sql_id")
