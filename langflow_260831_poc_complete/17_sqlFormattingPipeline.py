...
class NewType17SqlFormattingPipeline(Component):
    ...
    def run_pipeline(self) -> Data:
        ...
        logging.getLogger("smartmigrate.workflow").info("before run_pipeline", extra={"workflow_log": [0, "WORKFLOW", "17_PIPELINE", "INFO", "RUN_PIPELINE", "START", "before run_pipeline", 0]})
        try:
            ...
            logging.getLogger("smartmigrate.workflow").info("after run_pipeline", extra={"workflow_log": [0, "WORKFLOW", "17_PIPELINE", "INFO", "RUN_PIPELINE", "END", "after run_pipeline", 0]})
            return __log_result
        except Exception as exc:
            logging.getLogger("smartmigrate.workflow").error(f"error run_pipeline: {exc}", extra={"workflow_log": [0, "WORKFLOW", "17_PIPELINE", "ERROR", "RUN_PIPELINE", "ERROR", f"error run_pipeline: {exc}", 0]})
            raise
    ...
    def _mock_result(self, job: dict[str, Any], index: int) -> dict[str, Any]:
        ...
        rng = random.Random(f"SQL_FORMATTING:{space_nm}:{sql_id}:{index}")
        ...
        return {
            ...
            "log": f"[POC][SQL_FORMATTING][{index}] space_nm={space_nm}, sql_id={sql_id}, status={status}, trace_id=TEST-{rng.randint(1000, 9999)}",
        }
