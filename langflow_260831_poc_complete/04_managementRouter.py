...
Routes: DASHBOARD (read-only summary), CURRENT_PROGRESS (simple active/running work), JOB_QA (LLM agent answers job-related lookup/diagnostic/failure-analysis questions using read-only DB tools), STATUS_CHANGE (reset), CORRECT_SQL_INPUT (save user-supplied SQL), EXCEPTION (missing or ambiguous data).
...
- JOB_QA also includes chat requests for failure analysis, such as "전체 Fail 분석해줘", "최근 실패 원인 알려줘", or "SQL Tuning만 분석해줘".
...
JSON schema:
{"management_route":"DASHBOARD|CURRENT_PROGRESS|JOB_QA|STATUS_CHANGE|CORRECT_SQL_INPUT|EXCEPTION","target":{"work_type":"","map_id":"","sql_id":"","space_nm":"","sql_column":"","priority":5},"correct_sql":"","exception_message":"","reason":""}"""
...
class NewType04ManagementRouter(Component):
    ...
    def _normalize_decision(self, decision: dict[str, Any]) -> dict[str, Any]:
        ...
        if route not in {"DASHBOARD", "CURRENT_PROGRESS", "JOB_QA", "STATUS_CHANGE", "CORRECT_SQL_INPUT", "EXCEPTION"}:
    ...
    def _next_node(self, route: str) -> str:
        return {"DASHBOARD": "04_dashboard", "CURRENT_PROGRESS": "04_currentProgress", "JOB_QA": "04_jobQaAgent", "STATUS_CHANGE": "04_statusChange", "CORRECT_SQL_INPUT": "04_correctSqlInput", "EXCEPTION": "04_managementRouter"}.get(route, "04_dashboard")
